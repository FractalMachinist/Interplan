...
export async function RelDBLookupOutE(id){
	...
	const C_Latest = `{Deleted: false}`
	const C_Lookup = `MATCH (${C_Match})-[o ${C_Latest}]->()`
...
export async function RelDBLookupOutE_Latest(id){
	...
	const C_Match = `p {id: "${id}"}`
	const C_Latest = `{Deleted: false, Active: true}`
	const C_Lookup = `MATCH (${C_Match})-[o ${C_Latest}]->()`
	const C_Return = `${C_Lookup} RETURN collect(o.id) as OutE`
	...
	return Transaction(
		...
		const oe = Array.from(new Set(out_edges))
		console.log(`${id} RelDBLookupOutE_Latest with out_edges`, oe)
		return oe
	})
}
